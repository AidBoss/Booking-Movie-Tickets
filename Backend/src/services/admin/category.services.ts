...
const searchCategory = async (searchCriteria: Record<string, any>) => {
    try {
        const result = await Category.find(searchCriteria);
        if (result && result.length > 0) {
            return {
                status: 200,
        ...
        logger.error(`Lỗi khi tìm kiếm tỉnh/thành phố: ${e.message}`);
        return {
            status: 500,
            message: 'Lỗi server',
        };
